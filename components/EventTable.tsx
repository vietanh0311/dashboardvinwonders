...
export default function EventTable({ data, isLoading }: Props) {
  const sorted = [...data].sort((a, b) => b.videos - a.videos);

  return (
    <div className="rounded-xl border border-emerald-100 bg-white p-4 shadow-sm">
      <h3 className="mb-3 text-sm font-semibold text-gray-800">Theo sự kiện</h3>
        ...
        <table className="w-full text-left text-sm">
          <thead>
            <tr className="sticky top-0 bg-white text-xs uppercase text-gray-400">
              ...
              <th className="pb-2 font-medium">Creators</th>
            </tr>
          </thead>
          <tbody>
            {isLoading &&
            ...
            {!isLoading &&
              sorted.map((row) => (
                <tr key={row.name} className="border-t border-gray-100">
                  ...
                  <td className="whitespace-nowrap py-2 pr-2 text-gray-600">{formatNumber(row.views)}</td>
                  <td className="whitespace-nowrap py-2 text-gray-600">{formatNumber(row.creators)}</td>
                </tr>
              ))}
            ...
            {!isLoading && sorted.length === 0 && (
              <tr>
                <td colSpan={4} className="py-6 text-center text-sm text-gray-400">
                  Không có sự kiện nào trong khoảng thời gian đã chọn.
                </td>
              </tr>
            )}
          </tbody>
